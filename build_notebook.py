"""
Sestaví Jupyter notebook `maturita_reseni.ipynb` se 7 řešenými úlohami
z maturitního testu CERMAT (MA_2025_TS) podle zadání.

Notebook se generuje programově (nbformat), aby byl snadno reprodukovatelný:
    python build_notebook.py
    jupyter nbconvert --to notebook --execute --inplace maturita_reseni.ipynb
"""
...
# ----------------------------------------------------------------------------- title
md(r"""# Maturita z matematiky → Python
...
# ============================================================================= Úloha 1
md(r"""## Úloha 1 — Nákup borůvek (základní výpočet)
...
""")
code(r"""# Jednotkové ceny (Kč za kg)
...
md(r"""**Výsledek a interpretace.** Levnější je 1. prodejce (≈ 230,77 Kč/kg < 240,00 Kč/kg).
Za 600 Kč zákazník koupil 2,6 kg borůvek. Stejné množství by u dražšího (2.) prodejce stálo
**624 Kč**.""")

# ============================================================================= Úloha 4
md(r"""## Úloha 4 — Zjednodušení výrazu (algebra)
...
""")
code(r"""a = sp.symbols('a', real=True)
...
md(r"""**Výsledek a interpretace.** Determinant matice je nenulový, soustava má proto **jediné
řešení $x=18,\ y=3,\ z=-9$**. Numerické (NumPy) i přesné (SymPy) řešení se shodují.""")

# ============================================================================= Úloha 9
md(r"""## Úloha 9 — Kvadratická a lineární funkce (funkce a graf)
...
plt.show()""")
md(r"""**Výsledek a interpretace.** Hledaná lineární funkce je **$g:\ y=2x-2$**. Společné body jsou
...
# ============================================================================= Úloha 20
md(r"""## Úloha 20 — Zvýšení hladiny vody (geometrie)
...
""")
code(r"""r_kulicka = 3      # poloměr kuličky [cm]
...
print("Voda nepřeteče (nová hladina", 11 + float(dh), "cm <= 16 cm):",
      11 + float(dh) <= 16)""")
md(r"""**Výsledek a interpretace.** Hladina stoupne o **1,00 cm** (odpověď D). Objem kuličky
...
# ============================================================================= Úloha 15
md(r"""## Úloha 15 — Hod dvěma kostkami (pravděpodobnost)
...
""")
code(r"""omega = list(product(range(1, 7), range(1, 9)))   # všech 48 dvojic (a, b)
N = len(omega)
...
code(r"""# --- Ověření simulací (Monte Carlo) ---
...
# ============================================================================= Úloha 23
md(r"""## Úloha 23 — Exponenciální růst bakterií (model / růst)
...
""")
code(r"""t = sp.symbols('t', positive=True)
...
code(r"""# --- Graf růstu (násobek počátečního počtu během 24 h) ---
...
plt.show()""")
md(r"""**Výsledek a interpretace.** Počet bakterií vzroste 4× za **8 hodin** (odpověď D). Protože
...
with open("maturita_reseni.ipynb", "w", encoding="utf-8") as f:
    nbf.write(nb, f)
print("Hotovo: maturita_reseni.ipynb (", len(cells), "buněk )")
